...
from Chang_HYU_task1.models.rep_ctfa import get_model, reparameterize_model
from Chang_HYU_task1.models.multi_device_model import MultiDeviceModelContainer
from Chang_HYU_task1 import ckpts
...
def load_model(model_file_path: Optional[str] = None) -> Baseline:
    ...
    if model_file_path is None:
        with pkg_resources.path(ckpts, "hop_mel.ckpt") as ckpt_path:
            model_file_path = str(ckpt_path)
    else:
        with pkg_resources.path(ckpts, model_file_path) as ckpt_path:
            model_file_path = str(ckpt_path)
    ...
    model.model = reparameterize_model(model.model)  # Reparameterize for inference
    model.model.half()
    return model
